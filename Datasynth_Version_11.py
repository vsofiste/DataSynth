#em relação ao anterior este oculta as colunas encriptadas da visualização da segunda tabela
import pandas as pd
import numpy as np
...
def adicionar_ruido_branco_e_encriptar(dados, coluna, chave, ruido_nivel=1):
    ...
    dados[coluna] = dados[coluna].apply(lambda x: float(str(x).replace(',', '.')))
    ruido = Z0 * ruido_nivel * dados[coluna].std()
...
def formatar_colunas(dados):
    ...
    dados['Valor_Transacao'] = dados['Valor_Transacao'].apply(lambda x: float(str(x).replace(',', '.')))
...
if uploaded_file is not None:
    ...
    banco_dados_transacoes['Valor_Transacao'] = banco_dados_transacoes['Valor_Transacao'].apply(lambda x: float(str(x).replace(',', '.')))
    ...
    banco_dados_anonimizados['Valor_Transacao'] = banco_dados_anonimizados['Valor_Transacao'].apply(lambda x: float(str(x).replace(',', '.')))
    ...
    ruido_nivel = st.slider("Nível de Ruído", min_value=0, max_value=1000, value=50, step=10)
    ...
    # Gerar e visualizar a PDF do ruído branco
    ruido_branco = gerar_ruido_branco(1000)
    fig, ax = plt.subplots()
    sns.histplot(ruido_branco, bins=30, kde=True, ax=ax)
    mu, std = np.mean(ruido_branco), np.std(ruido_branco)
    xmin, xmax = ax.get_xlim()
    x = np.linspace(xmin, xmax, 100)
    p = (1/(np.sqrt(2*np.pi)*std)) * np.exp(-0.5*((x-mu)/std)**2)
    ax.plot(x, p, 'k', linewidth=2)
    ax.set_title("Histograma e Curva da Distribuição Normal")
    ax.set_xlabel("Valor")
    ax.set_ylabel("Densidade")
    st.pyplot(fig)
    st.write("Histograma e Curva da Distribuição Normal para o Ruído Branco")
else:
